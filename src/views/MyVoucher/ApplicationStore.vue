	...
	export default {
		...
		methods: {
			...
			loadingStoreData(param) {
				...
				this.$http.post('I_SCRM_WX_INTERFACE_015.action', params).then((res) => {
					let data = res.data,
		...
		created() {
			...
			this.activityId = localStorage.getItem('myVoucheActivityId'); //关联id
			this.coupId = localStorage.getItem('myVoucheCoupId'); //优惠券id
